...
.venv\Scripts\python.exe tools\generate_ideal_dataset.py `
  --output-root validation_data\ideal\procedural_pcb_v2 `
  --width 512 `
  --height 320 `
  --seed 17
```
...
이미 샘플을 내려받은 오프라인 환경에서는 `--no-download`를 추가한다. 결과 시작 화면은
`validation_results/source_grounded/source_grounded_index.html`이다. scanner-sim은
다른 장면·광학계·패턴군이므로 영상 기술 통계만 비교하며 정확도 pass/fail에는 사용하지
않는다. CS126MU profile 역시 장착 렌즈의 PSF, 감마, 왜곡, FPN을 측정한 것이 아니므로
실장비 전체 모델이 아니다. 자세한 증거 계층과 미적용 사유는
`docs/SOURCE_GROUNDED_VALIDATION.md`를 따른다.
...
`validation_results/automated_suite_v2/index.html`, 기계 판독용 요약은 같은 폴더의
