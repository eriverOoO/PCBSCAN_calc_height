...
새 리그는 프로젝터 광축을 수직으로 두고 카메라를 30도 기울입니다. 따라서 투영 keystone은 줄지만 카메라 영상에는 원근과 초점면 차이가 생깁니다. 이 디코더는 프로젝터 이미지를 사전에 왜곡하는 keystone pre-distortion을 기본 가정으로 사용하지 않습니다.

- 초점 문제는 기울어진 카메라 렌즈·리그 쪽에서 Scheimpflug 또는 수동 초점 정렬로 해결하는 것을 권장합니다. 디코더는 초점이 맞지 않은 영역을 소프트웨어 deblur로 복구하지 않습니다.
...
- 새 리그는 기존 기준면과 mm 보정을 재사용할 수 없습니다. 새 `scan_log.json`의 `rig_layout`, `camera_tilt_deg`, `projector_tilt_deg`는 calibration NPZ의 provenance와 일치해야 하며, 위치별 기하 변화에는 `p`, `d`, `l` 맵을 사용할 수 있습니다.
